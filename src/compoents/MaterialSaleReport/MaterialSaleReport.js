// http://localhost:3000/testreport/?sp=9&ifid=ToolsReport&pid=18305

import React, { useState, useEffect, useRef } from "react";
import Box from "@mui/material/Box";
import { DataGrid } from "@mui/x-data-grid";
import "./MaterialSaleReport.scss";
import OtherKeyData from "./MaterialSaleReport.json";
...
export default function MaterialSaleReport() {
  ...
  const getMasterData = async () => {
    ...
    const body = {
      con: `{"id":"","mode":"materialsalereport_master","appuserid":"${AllData?.uid}"}`,
      p: "",
  ...
  const fetchData = async (stat, end) => {
    ...
    setIsLoading(true);
    const body = {
      con: `{"id":"","mode":"materialsalereport","appuserid":"${AllData?.uid}"}`,
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div
        className="MaterialSaleReport_mainGridView"
