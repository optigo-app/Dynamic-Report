...
import "./MaterialMemoReport.scss";
import OtherKeyData from "./MaterialMemoReport.json";
...
export default function Materialmemoreport() {
  ...
  const fetchData = async (stat, end) => {
    ...
    setIsLoading(true);
    const body = {
      con: `{"id":"","mode":"materialmemoreport","appuserid":"${AllData?.uid}"}`,
  ...
  const uniqueCustomers = [
    "All",
    ...Array.from(new Set(originalRows?.map((row) => row?.material))),
  ];
  ...
  const handleClick = (params) => {
    ...
    window.parent.addTab(
      ...
      "<%=Auto_DomainPath%>salescrm/app/MaterialManagement_MaterialSale?invoiceno=" +
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div
        className="MaterialMemoReporttMain_mainGridView"
