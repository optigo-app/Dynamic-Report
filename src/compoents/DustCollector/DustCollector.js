...
import OtherKeyData from "./DustCollector.json";
...
export default function DustCollector() {
  ...
  const fetchData = async (stat, end) => {
    ...
    setIsLoading(true);
    const body = {
      con: `{"id":"","mode":"dustcollector","appuserid":"${AllData?.uid}"}`,
    ...
    const newFilteredRows = originalRows?.filter((row) => {
      ...
      if (isMatch && fromDate && toDate) {
        const dateColumn = columns.find(
          (col) => col.filterTypes && col.filterTypes.includes("DateRangeFilter")
        );
        if (dateColumn) {
    ...
    rows.forEach((row) => {
      ...
      if (!grouped[groupKey]) {
        ...
        for (const col of OtherKeyData?.rd1) {
          if (
            !col.GrupChekBox && 
            typeof newRow[col.field] === "number"
          ) {
...
// // http://localhost:3000/testreport/?sv=/e3tsaXZlLm9wdGlnb2FwcHMuY29tfX17ezIwfX17e3Rlc3Q3M319e3t0ZXN0NzN9fQ==/1&ifid=DustCollectorReport&pid=1603
...
//     setIsLoading(true);
//     const body = {
//       con: '{"id":"","mode":"dustcollector","appuserid":"amrut@eg.com"}',
//       p: `{"fdate":"${stat}","tdate":"${end}"}`,
//       f: "Task Management (taskmaster)",
//     };
...
//   const groupByField = (rows, groupField, emptyField) => {
//     const grouped = {};
//     rows.forEach((row) => {
//       const key = row[groupField];
//       if (!grouped[key]) {
//         grouped[key] = { ...row, [emptyField]: "-" }; 
//       } else {
//         grouped[key].netwt += row.netwt;
//         grouped[key].losswt += row.losswt;
//         grouped[key].dustwt += row.dustwt;
//         grouped[key].dustper += row.dustper;
//         grouped[key].refinewt += row.refinewt;
//       }
//     });
//     return Object.values(grouped).map((item, index) => ({ ...item, srNo: index + 1 }));
//   };
  

//   useEffect(() => {
//     if (!originalRows?.length) return;
//     let updatedRows = [...originalRows];
//     if (!grupEnChekBox["empbarcode"] && grupEnChekBox["dept"]) {
//       updatedRows = groupByField(originalRows, "dept", "empbarcode");
//     } 
//     else if (grupEnChekBox["empbarcode"] && !grupEnChekBox["dept"]) {
//       updatedRows = groupByField(originalRows, "empbarcode", "dept");
//     } 
//     else if (!grupEnChekBox["empbarcode"] && !grupEnChekBox["dept"]) {
//       const mergedRow = originalRows.reduce((acc, row) => ({
//         netwt: (acc.netwt || 0) + row.netwt,
//         losswt: (acc.losswt || 0) + row.losswt,
//         dustwt: (acc.dustwt || 0) + row.dustwt,
//         dustper: (acc.dustper || 0) + row.dustper,
//         refinewt: (acc.refinewt || 0) + row.refinewt,
//       }), {});
