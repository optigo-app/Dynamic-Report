...
import OtherKeyData from "./ConversionDeatil.json";
import "./ConversionDeatil.scss";
...
export default function ConversionDeatil() {
  ...
  const fetchData = async (stat, end) => {
    ...
    setIsLoading(true);
    const body = {
      con: `{"id":"","mode":"conversiondetails","appuserid":"${AllData?.uid}"}`,
            ...
            <div key={col.field}>
              <Accordion>
                ...
                <AccordionDetails className="gridMetalComboMain">
                  {uniqueValues.map((value) => (
                    <label key={value} style={{ display: "flex", gap: "5px" }}>
                      <input
                        type="checkbox"
                        value={value}
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div
        className="ConversionDeatil_report_main"
